...
export const useSoilHumidityPrediction = (
  potId: string,
  minutesAhead: number = 5
) => {
  const [prediction, setPrediction] = useState<SoilHumidityPrediction | null>(
    null
  );
  ...
  const fetchPrediction = useCallback(
    async (minutes: number = minutesAhead) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        console.log(
          `Fetched prediction for pot_1 (${minutes} min ahead):`,
          response.data
        );
      } catch (err) {
        setError(
